...
public class ChessMatch {
    ...
    public ChessPiece performChessMove(ChessPosition sourcePosition, ChessPosition targetPosition) {
        ...
        if (movedPiece instanceof Pawn) {
            if ((movedPiece.getColor() == Color.WHITE && target.getRow() == 0) || (movedPiece.getColor() == Color.BLACK && target.getRow() == 7)) {
                ...
                promoted = replasePromotedPiece("Q");
            }
        }

        check = (testChek(opponent(currentPlayer))) ? true : false;

        if (testChekMath(opponent(currentPlayer))) {
            checkMath = true;
        } else {
            nextTurn();
        }
    ...
    public ChessPiece replasePromotedPiece(String type) {
        if (promoted == null) {
    ...
    private boolean testChekMath(Color color) {
        if (!testChek(color)) {
            return false;
        }
        List<Piece> list = piecesOnTheBoard.stream().filter(x -> ((ChessPiece)x).getColor() == color).collect(Collectors.toList());
